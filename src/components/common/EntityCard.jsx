...
const EntityCard = ({
  ...
  const getSubtitle = () => {
    ...
    if (type === 'rider') return vehicleType;
  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm border hover:shadow-md transition-all duration-200 entity-card ${
      ...
      <div className={`space-y-2 mb-4 ${compact ? 'text-xs' : 'text-sm'}`}>
        ...
        {/* Rider specific info */}
        {type === 'rider' && currentOrders !== undefined && maxOrders && (
          <div className="flex items-center space-x-2 text-gray-600">
            <Package className={`${compact ? 'w-3 h-3' : 'w-4 h-4'} flex-shrink-0`} />
            <span className="truncate">
              {currentOrders}/{maxOrders} orders
              {totalDeliveries && ` • ${totalDeliveries} total`}
            </span>
          </div>
        )}
      </div>
      ...
      {type === 'rider' && currentOrders > 0 && (
        <div className={`mt-3 p-2 bg-orange-50 rounded-lg ${compact ? 'text-xs' : 'text-sm'}`}>
          <p className="text-orange-700 font-medium">
            Currently delivering {currentOrders} order{currentOrders !== 1 ? 's' : ''}
          </p>
        </div>
      )}
    </div>
